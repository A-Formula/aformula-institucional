/* Circular Gallery — porte vanilla (OGL/WebGL) do componente React CircularGallery.
   OGL self-hospedada em assets/js/ogl.mjs. Aplica na seção blog. */
import { Camera, Mesh, Plane, Program, Renderer, Texture, Transform } from "../assets/js/ogl.mjs";
...
class Media {
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font, onLoad }) {
    ...
    this.borderRadius = borderRadius; this.font = font; this.onLoad = onLoad;
    this.extra = 0; this.widthTotal = 0; this.width = 0; this.x = 0; this.scale = 1;
    this.padding = 2; this.speed = 0; this.isBefore = false; this.isAfter = false;
...
class App {
  constructor(container, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase }) {
    ...
    this.loaded = 0; this.allLoaded = false; this.live = false; this.failed = false;
    autoBind(this);
    this.createRenderer();
    this.createCamera();
    this.createScene();
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    ...
    this.medias = this.mediasImages.map((data, index) => new Media({
      ...
      onLoad: () => { this.loaded++; if (this.loaded >= this.mediasImages.length) this.allLoaded = true; },
    }));
  }
  ...
  update() {
    ...
    try {
      ...
      // textura bloqueada (ex.: file://) → reverte pro fallback estático
      this.failed = true;
      this.container.classList.remove("is-live");
      if (this.gl.canvas.parentNode) this.gl.canvas.parentNode.removeChild(this.gl.canvas);
      console.warn("CircularGallery: render WebGL falhou (provável file://). Fallback ativo.", e);
      return;
    }
    // só esconde o fallback depois de um render real com TODAS as texturas carregadas
    if (this.allLoaded && !this.live) { this.live = true; this.container.classList.add("is-live"); }
...
/* ---------- init ---------- */
const ITEMS = [
  { image: "assets/img/blog5.jpg", text: "Creatina na vida da mulher" },
  { image: "assets/img/blog1.jpg", text: "Início do Outono" },
  { image: "assets/img/blog2.jpg", text: "Suplementação 2026" },
  { image: "assets/img/blog3.jpg", text: "Skincare com inteligência" },
  { image: "assets/img/blog4.jpg", text: "Imunidade em alta" },
];
...
function initBlogGallery() {
  const el = document.getElementById("blogGallery");
  if (!el) return;
  // file:// bloqueia texturas WebGL + ES modules → mantém o fallback estático
  ...
  try {
    new App(el, {
      items: ITEMS,
      bend: 2.5,
      textColor: cs.color || "#00424b",
      borderRadius: 0.06,
      font,
      scrollSpeed: 2,
      scrollEase: 0.05,
    });
    // o fallback só é escondido dentro de App.update(), após um render real com texturas
  } catch (e) {
    console.warn("CircularGallery indisponível, usando fallback:", e);
  }
}

if (document.readyState !== "loading") initBlogGallery();
else document.addEventListener("DOMContentLoaded", initBlogGallery);

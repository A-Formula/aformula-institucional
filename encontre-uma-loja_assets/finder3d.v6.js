...
(function () {
  ...
  /* ---------- página própria da unidade (v6 — v5 + ícones nos botões do card/popup) ----------
     Existe uma página em /encontre-uma-loja/{slug}/ para TODA unidade aberta — a
     mesma regra do gerador (scripts/build-lojas.mjs usa /em breve/i sobre o nome).
     "Em breve" não tem página de propósito: sem atendimento, a página mentiria.

     DECISÃO DO OPERADOR 2026-08-18: o CARD INTEIRO abre a página da unidade, e o botão
     "Ver a loja" foi REMOVIDO ("não gostei do botão, ficou horrível"). O clique no corpo
     do card navega; os links de dentro (WhatsApp, Como chegar, compartilhar) seguem
     funcionando porque cada um chama event.stopPropagation().
     Consequência aceita: o trilho deixou de servir pra navegar o mapa no clique — quem
     move o mapa agora é o pin, a busca por CEP e os filtros.
     O título continua sendo um <a> de verdade: é o que dá acesso por TECLADO e o que o
     robô de busca segue (um <article> com onclick não é link pra ninguém). */
  var CARD_ABRE_PAGINA = true;

  function pagUnidade(s) {
    return (!isSoon(s) && s.slug) ? "/encontre-uma-loja/" + s.slug : null;
  }
  ...
  /* Pino de localização para o botão "Como chegar". No mobile o card tem 216px de área útil
     e os 3 botões com rótulo somavam exatamente 216 — folga zero, e "Como chegar" quebrava em
     2 linhas (medido em 390px). Com o ícone, o rótulo pode sumir no mobile e a ação continua
     legível: pino = rota é convenção, e o aria-label garante o leitor de tela. */
  var PIN_ICON = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>';
  ...
  function popupFor(s, dist) {
    ...
    return new maplibregl.Popup({ offset: 22, closeButton: true, className: "af-pop", maxWidth: "290px" })
      ...
      .setHTML(
        '<div class="af-pop__in"><strong>' +
        (pagUnidade(s)
          ? '<a class="railcard__link" href="' + pagUnidade(s) + '">' + cardTitle(s) + "</a>"
          : cardTitle(s)) + "</strong>" +
        ...
        (isSoon(s)
          ...
          : '<a class="rota" href="' + mapsLink(s) + '" target="_blank" rel="noopener" aria-label="Como chegar">' + PIN_ICON + '<span class="railcard__rot">Como chegar</span></a>' +
            (wa ? '<a class="wa" href="' + wa + '" target="_blank" rel="noopener">' + WA_ICON + '<span>WhatsApp</span></a>' : "") +
            shareBtn(s)) + "</div></div>"
      );
  }
  ...
  function railCard(s, dist) {
    ...
    var pag = pagUnidade(s);
    // Título é link de verdade (acesso por teclado + rastreável). stopPropagation pra
    // não disparar DUAS navegações: a do <a> e a do clique no card.
    var titulo = pag
      ? '<a class="railcard__link" href="' + pag + '" onclick="event.stopPropagation()">' + cardTitle(s) + "</a>"
      : cardTitle(s);
    el.innerHTML =
      '<div class="railcard__top"><h3>' + titulo + "</h3>" +
      ...
      (isSoon(s)
        ...
        : '<a class="rota" href="' + mapsLink(s) + '" target="_blank" rel="noopener" aria-label="Como chegar" onclick="event.stopPropagation()">' + PIN_ICON + '<span class="railcard__rot">Como chegar</span></a>' +
          (wa ? '<a class="wa" href="' + wa + '" target="_blank" rel="noopener" onclick="event.stopPropagation()">' + WA_ICON + '<span>WhatsApp</span></a>' : "") +
          shareBtn(s)) +
      "</div>";
    el.addEventListener("click", function () {
      if (CARD_ABRE_PAGINA && pag) { window.location.href = pag; return; }
      focusStore(s, true);
    });
    return el;
  }

...
(function () {
  ...
  /* ---------- página própria da unidade (v4) ----------
     ...
     ⚠️ CARD_ABRE_PAGINA: com `false` (padrão), clicar no card segue voando o mapa
     até a unidade e a navegação fica nos links explícitos (título + "Ver a loja").
     Com `true`, o clique em qualquer parte do card navega pra página — o que TIRA do
     trilho a função de navegar o mapa. Decisão de UX pendente do operador. */
  var CARD_ABRE_PAGINA = false;
    ...
    return new maplibregl.Popup({ offset: 22, closeButton: true, className: "af-pop", maxWidth: "290px" })
      ...
      .setHTML(
        ...
        (isSoon(s)
          ...
          : (pagUnidade(s) ? '<a class="unidade" href="' + pagUnidade(s) + '">Ver a loja</a>' : "") +
  ...
  function railCard(s, dist) {
    ...
    // Título vira link pra página da unidade quando ela existe. stopPropagation pra
    // não disparar o voo do mapa junto com a navegação.
    var titulo = pag
    ...
    el.innerHTML =
      ...
      (isSoon(s)
        ...
        : (pag ? '<a class="unidade" href="' + pag + '" onclick="event.stopPropagation()">Ver a loja</a>' : "") +
